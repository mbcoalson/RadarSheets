...
def generate_company_radar_plot(valid_rows, categories, company_name, output_path):
    """
    Generates a radar plot for a single company with multiple evaluators.
        ...
        try:
            ...
            for company_name, company_rows in grouped_data.items():
                try:
                    ...
                    output_path = os.path.join(output_dir, f"{company_name}_{sheet_name}_radar.html")
                    generate_company_radar_plot(valid_rows, categories, company_name, output_path)
